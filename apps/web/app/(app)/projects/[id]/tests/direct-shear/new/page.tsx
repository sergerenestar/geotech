...
import { useCreateDsTest } from '@/hooks/useDsTests';

type Reading = { dispDiv: string; prDiv: string };
type Stage = { normalStress: string; readings: Reading[] };

export default function NewDsTestPage() {
  const { id: projectId } = useParams<{ id: string }>();
  const router = useRouter();
  const createTest = useCreateDsTest();
  ...
  const [stages, setStages] = useState<Stage[]>([
    { normalStress: '', readings: [{ dispDiv: '', prDiv: '' }] },
    { normalStress: '', readings: [{ dispDiv: '', prDiv: '' }] },
    { normalStress: '', readings: [{ dispDiv: '', prDiv: '' }] },
  ]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/projects/${projectId}/tests/direct-shear/${test.id}`);
    } catch (err: unknown) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Nouvel essai — Cisaillement direct (D-3080)</h1>
