...
import { useUcTest, useUpdateUcTest } from '@/hooks/useUcTests';

export default function EditUcTestPage() {
  const { id: projectId, testId } = useParams<{ id: string; testId: string }>();
  const router = useRouter();
  const { data: test, isLoading } = useUcTest(testId);
  const updateTest = useUpdateUcTest();
  ...
  useEffect(() => {
    if (test && !ready) {
      setH0(String(test.initialHeightMm));
      setD0(String(test.initialDiameterMm));
      setDialFactor(String(test.dialGaugeFactorMmPerDiv));
      setCalibFactor(String(test.calibrationFactorNPerDiv));
      setFailureMode(test.failureMode ?? '');
      setNotes(test.notes ?? '');
      setReadings(test.readings.map(r => ({
        defDiv: String(r.deformationDiv),
  ...
  const A0 = D0 > 0 ? Math.PI * Math.pow(D0 / 10, 2) / 4 : 0;
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Modifier — Compression non confinée (D-2166)</h1>
