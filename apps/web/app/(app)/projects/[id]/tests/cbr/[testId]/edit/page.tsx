...
import { useCbrTest, useUpdateCbrTest, CbrIntensityInput, SwellingInput, PenetrationInput } from '@/hooks/useCbrTests';
...
export default function EditCbrTestPage() {
  const { id: projectId, testId } = useParams<{ id: string; testId: string }>();
  const router = useRouter();
  const { data: test, isLoading } = useCbrTest(testId);
  const updateTest = useUpdateCbrTest();
  ...
  const [intensities, setIntensities] = useState<Record<number, IntensityState>>({
    55: emptyIntensity(), 25: emptyIntensity(), 10: emptyIntensity(),
  });
  ...
  useEffect(() => {
    if (test && !ready) {
      ...
      setMoldVolumeCm3(test.moldVolumeCm3 != null ? String(test.moldVolumeCm3) : '2332');
      const latestSg = sgTests.find(t => t.gsAverage != null)?.gsAverage;
      setSpecificGravity(latestSg != null ? String(latestSg.toFixed(4)) : String(test.specificGravity));
      setProctorGdmax(test.proctorGdmaxTm3 != null ? String(test.proctorGdmaxTm3) : '');
      setProctorOmc(test.proctorOmcPct != null ? String(test.proctorOmcPct) : '');
      setRingCoeff(String(test.ringCoefficient));
      setNotes(test.notes ?? '');

      const newIntensities: Record<number, IntensityState> = {
        55: emptyIntensity(), 25: emptyIntensity(), 10: emptyIntensity(),
      };
      for (const intensity of test.intensities) {
        const swelling: Record<number, string> = {};
        SWELLING_HOURS.forEach(h => { swelling[h] = ''; });
        for (const sr of intensity.swellingReadings) {
          swelling[sr.hours] = sr.readingMm != null ? String(sr.readingMm) : '';
        }
        const penetration: Record<number, string> = {};
        PENETRATION_MM.forEach(mm => { penetration[mm] = ''; });
        for (const pr of intensity.penetrationReadings) {
          const key = PENETRATION_MM.find(mm => Math.abs(mm - pr.penetrationMm) < 0.001);
          if (key != null) penetration[key] = pr.ringReading != null ? String(pr.ringReading) : '';
        }
        newIntensities[intensity.blows] = {
          massMoldG: intensity.massMoldG != null ? String(intensity.massMoldG) : '',
          massWetG: intensity.massWetG != null ? String(intensity.massWetG) : '',
          massDryG: intensity.massDryG != null ? String(intensity.massDryG) : '',
          cbr25mm: intensity.cbr25mm != null ? String(intensity.cbr25mm) : '',
          cbr50mm: intensity.cbr50mm != null ? String(intensity.cbr50mm) : '',
          swelling,
          penetration,
        };
      }
  ...
  function updateIntensity(blows: number, field: keyof IntensityState, value: string) {
    setIntensities(prev => ({ ...prev, [blows]: { ...prev[blows], [field]: value } }));
  }
  function updateSwelling(blows: number, hours: number, value: string) {
    setIntensities(prev => ({ ...prev, [blows]: { ...prev[blows], swelling: { ...prev[blows].swelling, [hours]: value } } }));
  }
  function updatePenetration(blows: number, mm: number, value: string) {
    setIntensities(prev => ({ ...prev, [blows]: { ...prev[blows], penetration: { ...prev[blows].penetration, [mm]: value } } }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const intensityPayload: CbrIntensityInput[] = BLOW_LEVELS.map(blows => {
      ...
      const penetrationReadings: PenetrationInput[] = PENETRATION_MM
        ...
        .map(mm => ({ penetrationMm: mm, ringReading: parseFloat(intensity.penetration[mm]) || undefined }));
      return {
        blows,
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Modifier — Essai CBR (NF P94-078)</h1>
            ...
            <Card key={blows} className="p-6 space-y-4">
              ...
              <div>
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr>{SWELLING_HOURS.map(h => <th key={h} className={th}>{h}h</th>)}</tr>
                  </thead>
                  <tbody>
                    <tr>
                      {SWELLING_HOURS.map(h => (
                        <td key={h} className={td}>
                          <input type="number" step="0.01" value={intensity.swelling[h]}
                  ...
                  <table className="text-sm border-collapse" style={{ minWidth: '600px' }}>
                    <thead>
                      <tr>
                        ...
                        {PENETRATION_MM.map(mm => <th key={mm} className={th}>{mm.toFixed(3)}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        ...
                        {PENETRATION_MM.map(mm => (
                          <td key={mm} className={td}>
                            <input type="number" step="0.01" value={intensity.penetration[mm]}
