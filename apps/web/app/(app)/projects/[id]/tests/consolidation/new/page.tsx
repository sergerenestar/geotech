...
import { useCreateConsolTest } from '@/hooks/useConsolTests';
...
export default function NewConsolTestPage() {
  const { id: projectId } = useParams<{ id: string }>();
  const router = useRouter();
  const createTest = useCreateConsolTest();
  ...
  const [stages, setStages] = useState<Stage[]>([
    { loadType: 'LOADING', sigmaKpa: '', readings: [{ timeMin: '0', dialMm: '' }, { timeMin: '0.25', dialMm: '' }, { timeMin: '1', dialMm: '' }, { timeMin: '4', dialMm: '' }, { timeMin: '15', dialMm: '' }, { timeMin: '60', dialMm: '' }, { timeMin: '240', dialMm: '' }, { timeMin: '1440', dialMm: '' }] },
  ]);

  function addStage() {
    setStages(ss => [...ss, {
      loadType: 'LOADING', sigmaKpa: '',
      readings: [{ timeMin: '0', dialMm: '' }, { timeMin: '0.25', dialMm: '' }, { timeMin: '1', dialMm: '' }, { timeMin: '4', dialMm: '' }, { timeMin: '15', dialMm: '' }, { timeMin: '60', dialMm: '' }, { timeMin: '240', dialMm: '' }, { timeMin: '1440', dialMm: '' }],
    }]);
  }
  ...
  function addReading(si: number) {
    setStages(ss => ss.map((s, i) => i === si ? { ...s, readings: [...s.readings, { timeMin: '', dialMm: '' }] } : s));
  }

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError('');
    if (!diameterMm || !heightMm || !e0) { setError('Diamètre, hauteur initiale et indice des vides e0 requis.'); return; }
    const validStages = stages.filter(s => s.sigmaKpa && s.readings.some(r => r.dialMm));
    if (validStages.length === 0) { setError('Au moins un palier de charge est requis.'); return; }
    try {
      ...
      router.push(`/projects/${projectId}/tests/consolidation/${test.id}`);
    } catch (err: unknown) {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Nouvel essai — Consolidation (D-2435)</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <Card className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <Input label="Indice des vides initial e₀" type="number" step="0.001" required value={e0} onChange={e => setE0(e.target.value)} />
            <Input label="Densité relative Gs" type="number" step="0.001" value={gs} onChange={e => setGs(e.target.value)} />
            <div>
        ...
        <Button type="button" variant="secondary" onClick={addStage}>+ Ajouter un palier</Button>
