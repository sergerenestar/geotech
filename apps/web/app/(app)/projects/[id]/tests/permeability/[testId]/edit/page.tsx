...
import { usePermTest, useUpdatePermTest } from '@/hooks/usePermTests';
...
export default function EditPermTestPage() {
  ...
  useEffect(() => {
    if (test && !ready) {
      ...
      setLengthMm(String(test.specimenLengthMm));
      setTempC(String(test.waterTemperatureC));
      setHeadCm(test.headCm != null ? String(test.headCm) : '');
      setStandpipeArea(test.standpipeAreaCm2 != null ? String(test.standpipeAreaCm2) : '');
      setNotes(test.notes ?? '');
      setReadings(test.readings.map(r => ({
        flowVolumeMl: r.flowVolumeMl != null ? String(r.flowVolumeMl) : '',
        initialHeadCm: r.initialHeadCm != null ? String(r.initialHeadCm) : '',
        finalHeadCm: r.finalHeadCm != null ? String(r.finalHeadCm) : '',
        elapsedTimeS: String(r.elapsedTimeS),
      })));
      setReady(true);
    }
  }, [test, ready]);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Modifier — Perméabilité (D-2434)</h1>
          ...
          <div className="flex gap-4">
            {(['CONSTANT_HEAD', 'FALLING_HEAD'] as const).map(type => (
              <label key={type} className="flex items-center gap-2 cursor-pointer">
                <input type="radio" name="testType" value={type}
                  checked={testType === type} onChange={() => setTestType(type)} className="accent-brand-600" />
        ...
        <Card className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            {testType === 'CONSTANT_HEAD' && (
              <Input label="Charge h (cm)" type="number" step="0.1" value={headCm} onChange={e => setHeadCm(e.target.value)} />
            )}
            {testType === 'FALLING_HEAD' && (
              <Input label="Aire tube a (cm²)" type="number" step="0.01" value={standpipeArea} onChange={e => setStandpipeArea(e.target.value)} />
            )}
          </div>
          {aCm2 > 0 && (
            <p className="text-sm text-gray-500">Aire: {aCm2.toFixed(3)} cm² | L: {lCm.toFixed(2)} cm</p>
          )}
        </Card>
        ...
        <Card className="p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button type="button" variant="secondary"
              onClick={() => setReadings(rs => [...rs, { flowVolumeMl: '', initialHeadCm: '', finalHeadCm: '', elapsedTimeS: '' }])}>
              + Lecture
            </Button>
          </div>
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-gray-50 text-gray-600 text-xs uppercase">
                ...
                {testType === 'CONSTANT_HEAD' ? <th className={col}>Q (mL)</th> : <><th className={col}>h1 (cm)</th><th className={col}>h2 (cm)</th></>}
              ...
              {readings.map((r, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    {testType === 'CONSTANT_HEAD' ? (
                      <td className={col}><input type="number" step="0.1" value={r.flowVolumeMl} onChange={e => updateReading(i, 'flowVolumeMl', e.target.value)} className="w-20 border-0 outline-none text-right bg-transparent" /></td>
                    ) : (
                      <>
                        <td className={col}><input type="number" step="0.1" value={r.initialHeadCm} onChange={e => updateReading(i, 'initialHeadCm', e.target.value)} className="w-16 border-0 outline-none text-right bg-transparent" /></td>
                        <td className={col}><input type="number" step="0.1" value={r.finalHeadCm} onChange={e => updateReading(i, 'finalHeadCm', e.target.value)} className="w-16 border-0 outline-none text-right bg-transparent" /></td>
                      </>
                    )}
                    <td className={col}><input type="number" step="1" value={r.elapsedTimeS} onChange={e => updateReading(i, 'elapsedTimeS', e.target.value)} className="w-20 border-0 outline-none text-right bg-transparent" /></td>
                    <td className={`${col} text-right font-medium text-brand-700`}>{k != null ? k.toExponential(2) : '—'}</td>
