...
import { useCreateSgTest, SgDeterminationInput } from '@/hooks/useSgTests';
...
export default function NewSgTestPage() {
  const { id: projectId } = useParams<{ id: string }>();
  const router = useRouter();
  const createTest = useCreateSgTest();

  const [notes, setNotes] = useState('');
  const [error, setError] = useState('');
  const [rows, setRows] = useState([
    { flask: '', massDry: '', massFlaskWater: '', massFlaskSoilWater: '', temp: '20' },
    { flask: '', massDry: '', massFlaskWater: '', massFlaskSoilWater: '', temp: '20' },
  ]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const test = await createTest.mutateAsync({ projectId, notes: notes || undefined, determinations });
      router.push(`/projects/${projectId}/tests/specific-gravity/${test.id}`);
    } catch (err: unknown) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Nouvel essai — Densité relative (D-854)</h1>
